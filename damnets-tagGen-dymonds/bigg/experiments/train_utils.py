    ...
    **kwargs
    ...
    for st_delta in cache_stages[:-1]:
        ...
        with torch.no_grad():
            ...
            _, new_states = model.forward_row_summaries(
                graph_ids,
    ...
    for i in range(len(cache_stages) - 1, -1, -1):
        ...
        ll, cur_states = model.forward_train(
            graph_ids, list_node_starts=[node_st], num_nodes=cur_num, prev_rowsum_states=prev_states, **kwargs
        )
        tot_ll += ll.item()
        loss = -ll * loss_scale
        ...
        for idx in pbar:
            ...
            if rank == 0:
                pbar.set_description(
                    "epoch %.2f, loss: %.4f" % (epoch + (idx + 1) / cmd_args.epoch_save, loss)
                )
