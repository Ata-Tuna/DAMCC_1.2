...
if __name__ == "__main__":
    ...
    if cmd_args.eval_folder is not None:
        ...
        test_graphs, stats = load_graphs(
            os.path.join(eval_dir, "test-graphs.pkl"), os.path.join(eval_dir, "test-graph-stats.pkl")
        )
        out_dir = os.path.join(
            cmd_args.save_dir,
            "%s-pred_formulas-e-%d-g-%.2f"
        ...
        for _ in range(1):
            for g, stat in tqdm(zip(test_graphs, stats)):
                ...
                with torch.no_grad():
                    _, pred_edges, _ = model(
                        stat[0],
                        lb_list=degree_list,
                        ub_list=degree_list,
                        col_range=(0, stat[1]),
                        num_nodes=len(g),
                        display=cmd_args.display,
                    )
                    ...
                    print(
                        ...
                        len(true_edges),
                        "precision",
                        len(common) / len(pred_edges),
                        "recall",
                        len(common) / len(true_edges),
                    )
    ...
    for epoch in range(cmd_args.num_epochs):
        ...
        for idx in pbar:
            ...
            if cmd_args.blksize < 0 or num_nodes <= cmd_args.blksize:
                ll, _ = model.forward_train(
                    batch_indices, list_col_ranges=[(0, stats[i][1]) for i in batch_indices]
                )
                loss = -ll / num_nodes
                loss.backward()
                loss = loss.item()
