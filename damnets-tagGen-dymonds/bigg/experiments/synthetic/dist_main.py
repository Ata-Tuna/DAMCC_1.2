        ...
        for e_it in pbar:
            ...
            for stage in range(num_stages):
                ...
                with torch.no_grad():
                    fn_hc_bot, h_buf_list, c_buf_list = model.forward_row_trees(
                        graph_ids, list_node_starts=[local_st], num_nodes=local_num
                    )
                    if stage and rank == 0:
                    ...
                    _, next_states = model.row_tree.forward_train(
                        *(fn_hc_bot(0)), h_buf_list[0], c_buf_list[0], *states_prev
                    )
            ...
            if rank == 0:
                ...
                pbar.set_description(
                    "epoch %.2f, loss: %.4f" % (epoch + (e_it + 1) / cmd_args.epoch_save, loss)
                )
