...
class _tree_lib(object):
    ...
    def InsertGraph(self, labels, nx_g, bipart_stats=None):
        ...
        self.lib.AddGraph(
            ...
            ctypes.c_void_p(ctype_g.edge_pairs.ctypes.data),
            ctypes.c_void_p(ctype_g.edge_signs.ctypes.data),
            n,
            m,
        )
        return gid

    def PrepareMiniBatch(
        self, list_gids, list_node_start=None, num_nodes=-1, list_col_ranges=None, new_batch=True
    ):
        ...
        for d in range(max_d):
            ...
            if num_nodes == 0:
                ...
                if self.device == torch.device("cpu"):
                    ...
                    dev = 1
                self.lib.SetBinaryFeat(
                    d, ctypes.c_void_p(pos_feat.data_ptr()), ctypes.c_void_p(neg_feat.data_ptr()), dev
                )
    ...
    def PrepareRowSummary(self):
        ...
        return (
            ...
            last_ids,
            next_ids,
            torch.tensor(np_pos, dtype=torch.float32).to(self.device),
        )
    ...
    def GetChLabel(self, lr, depth=-1, dtype=None):
        if lr == 0:  # == root
            ...
            self.lib.GetChMask(lr, depth, ctypes.c_void_p(has_ch.ctypes.data))
            num_ch = np.empty((n,), dtype=np.int32)
            self.lib.GetNumCh(lr, depth, ctypes.c_void_p(num_ch.ctypes.data))
