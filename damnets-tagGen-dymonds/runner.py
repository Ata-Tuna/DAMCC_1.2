            ...
            for ts in test_list:
                ...
                for g_ in ts[:-1]:
                    if self.args.series_generation:
                        g = nx.Graph(samples_ts[-1])
                    else:
                        g = nx.Graph(g_)
                    
                    ## Convert to adjacency for GNN
                    edges_l = list(map(lambda x: [x[0], x[1]], g.edges))
                    edges = torch.Tensor(edges_l).T.int().to(device)
                    ...
                    add_ratio.append(
                        sum([not g.has_edge(*edge) for edge in new_edges]) / (len(new_edges) + 1)
                    )
