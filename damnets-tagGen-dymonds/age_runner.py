...
class Runner:
    ...
    def process_ablation(self, graphs):
        ...
        diffs = [compute_adj_delta(g, abs=True) for g in graphs]
    ...
    def train(self):
        ...
        opt = optim.Adam(
            model.parameters(), lr=self.exp_args.train.lr, weight_decay=self.exp_args.train.weight_decay
        )
        ...
        for epoch in range(self.exp_args.train.epochs):
            ...
            for batch in train_loader:
                ...
                if iter_count % self.args.experiment.train.display_iters == 0:
                    print(
                        f"Iteration: {iter_count} | Train Loss: {loss: .3f} | "
                        f"Epoch: {epoch} | "
    ...
    def sample_nts(self, graphs, model):
        ...
        loader = DataLoader(
            sampler, collate_fn=sampler.collate_fn, batch_size=self.args.experiment.test.batch_size
        )
    ...
    def test(self):
        ...
        model = AGE(self.args).to(device)
        best_markov_file = os.path.join(self.args.model_save_dir, "val.pt")
        # print(f'Best Model File : {best_markov_file}')
        load_model(model, best_markov_file)
        sampled_ts_list = []
        model.eval()
        ...
        with torch.no_grad():
            for ts in test_list:
                samples_ts = [ts[0]]
                for g in ts[:-1]:
                    if self.args.series_generation:
                        graph = samples_ts[-1]
                    else:
                        graph = g
                    
                    adj = np.tril(nx.to_numpy_array(graph), k=-1)
                    data = {"adj": torch.tensor(adj, dtype=torch.float32).unsqueeze(0), "is_sampling": True}
                    adj_pred = model(data).squeeze(0)
                    adj_pred = (adj_pred + adj_pred.T).cpu().numpy()
                    if is_ablation:
                        prev_adj = nx.to_numpy_array(graph)
