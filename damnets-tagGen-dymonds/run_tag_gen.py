...
def run_tag_gen(graphs_file, output_dir, real_dataset=False, T=1):
    if not real_dataset:
        ...
        el_fstr = os.path.join(output_dir, "train_edgelists")
    print("----------------------- done create edgelists -------------------")
    train_test_tag_gen(T, el_fstr)
    print("----------------------- done traintest -------------------")
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="A script to run the DYMOND baseline model")
    parser.add_argument(
        "-p", "--graphs_file", type=str, help="The path to the graphs on which to train DYMOND."
    )
    parser.add_argument(
        "-o", "--output_dir", type=str, help="The directory in which to place the sampled graphs from DYMOND."
    )
    parser.add_argument("-r", "--real_dataset", type=bool, help="real dataset.", default=False)
    parser.add_argument("-nbg", "--nbGraphs", type=int, help="number of graphs", default=1)
...
if __name__ == "__main__":
    ...
    run_tag_gen(args.graphs_file, args.output_dir, args.real_dataset, args.nbGraphs)
