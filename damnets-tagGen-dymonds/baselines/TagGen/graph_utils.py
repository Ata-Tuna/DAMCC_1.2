...
def data_process(
    ...
    biased,
    time_windows=2,
    data_directory="./data/DBLP/edgelist.txt",
    output_directory="./data/DBLP",
    directed=False,
):
    node_dict, node_index, original_node_index, node_value, node_level = preprocess_edgelist(
        data_directory, interval, directed, time_windows
    )
    if not os.path.exists("{}/{}_emb".format(output_directory, args.data)):
        os.system(
            "python ./baselines/TagGen/deepwalk/main.py --representation-size {} --input {} --output {}/{}_emb".format(
    ...
    temporal_random_walks(
        node_dict, node_similarity, degree, output_directory, biased, node_index, node_value
    )
...
def unbiased_temporal_neighbor_selection_with_prob(
    neighbor_list, node_similarity=None, k=None, node_index=None
):
    """
    input:
...
def biased_temporal_neighbor_selection_with_prob(neighbor_list, node_similarity, k, node_index):
    ...
    """
    prob_list = [
        node_similarity[node_index[k], node_index["{}_{}".format(item[0], item[1])]] for item in neighbor_list
    ]
    ...
    with open(data_directory, "r") as f:
        with open(data_directory[:-4] + "_new" + data_directory[-4:], "w") as f_out:
            for line in f:
                ...
                f_out.write(
                    "{} {} {}\n".format(
                        node_index["{}_{}".format(nodes[1], nodes[2])],
                        node_index["{}_{}".format(nodes[0], nodes[2])],
                        nodes[2],
                    )
                )
                f_out.write(
