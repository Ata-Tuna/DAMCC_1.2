        ...
        for j in range(config.sample_time):
            ...
            if action == 0:
                ...
                for k in range(n):
                    ...
                    candidate_sequence_role_level[k] = np.concatenate(
                        [
                            sequence_role_level[0, :ind],
                            sampling_role_level_emb[k].reshape(1, config.d_model),
                            sequence_role_level[0, ind:],
                        ],
                        axis=0,
                    )
                    candidate_sequence_node_level[k] = np.concatenate(
                        [
                            sequence_node_level[0, :ind],
                            sampling_node_level_emb[k].reshape(1, config.d_model),
                            sequence_node_level[0, ind:],
                        ],
                        axis=0,
                    )
                candidate_sequence_role_level = torch.from_numpy(candidate_sequence_role_level).to(
                    model.device
                )
                candidate_sequence_node_level = torch.from_numpy(candidate_sequence_node_level).to(
                    model.device
                )
                if candidate_sequence_role_level.shape[1] <= 5:
                    ...
                    y_pred = model(
                        candidate_sequence_role_level[:, start_ind:end_ind, :],
                        candidate_sequence_node_level[:, start_ind:end_ind, :],
                    )
                accept_prob = y_pred.cpu().data[:, 1]
                max_accept_prob, indices = torch.max(accept_prob, 0)
                if max_accept_prob > config.threshold:
                    sequence_role_level = (
                        candidate_sequence_role_level[indices]
                        .cpu()
                        .numpy()
                        .reshape(1, len(key_nodes) + 1, config.d_model)
                    )
                    sequence_node_level = (
                        candidate_sequence_node_level[indices]
                        .cpu()
                        .numpy()
                        .reshape(1, len(key_nodes) + 1, config.d_model)
                    )
            ...
            if action == 1:
                ...
                if len(key_nodes) <= min_length:
                    ...
                    candidate_sequence_role_level = np.zeros(
                        (len(key_nodes), len(key_nodes) - 1, config.d_model)
                    )
                    candidate_sequence_node_level = np.zeros(
                        (len(key_nodes), len(key_nodes) - 1, config.d_model)
                    )
                    ...
                    for k in range(len(key_nodes)):
                        ...
                        candidate_sequence_role_level[k] = np.concatenate(
                            [sequence_role_level[0, :k], sequence_role_level[0, k + 1 :]], axis=0
                        )
                        candidate_sequence_node_level[k] = np.concatenate(
                    ...
                    if max_accept_prob > config.threshold and choose_action([0.5, 0.5]):
                        ...
                        key_nodes = candidate_key_nodes[indices]
                        node_sequence = np.concatenate(
                            [node_sequence[:indices], node_sequence[indices + 1 :]], axis=0
                        )
                    else:
                        action = 2
            if action == 2:
...
def main(args, config, output_directory):
    if args.mode:
        train_dataset = data_loader(
            output_directory, config.embedding, config.node_embedding, "train", args.mode
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-b", dest="biased", action="store_true", help="biased or unbiased, default is biased"
    )
