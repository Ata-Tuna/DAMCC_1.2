...
def process(args):
    ...
    if data_size < args.max_memory_data_size:
        ...
        print(
            "Data size {} is larger than limit (max-memory-data-size: {}).  Dumping walks to disk.".format(
...
def main():
    parser = ArgumentParser(
        "deepwalk", formatter_class=ArgumentDefaultsHelpFormatter, conflict_handler="resolve"
    )
    ...
    parser.add_argument(
        "--number-walks", default=10, type=int, help="Number of random walks to start at each node"
    )
    ...
    parser.add_argument(
        "--walk-length", default=40, type=int, help="Length of the random walk started at each node"
    )
