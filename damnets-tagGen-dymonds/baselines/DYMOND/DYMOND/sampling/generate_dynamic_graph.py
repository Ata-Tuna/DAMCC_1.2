...
def get_active_nodes(num_timesteps, length_timestep, num_nodes, node_rate, gen_dir):
    ...
    if not file_exists(active_nodes_file):
        ...
        X = [np.random.exponential(node_rate["scale"] + node_rate["loc"]) for n in range(num_nodes)]
...
def get_active_triplets(new_nodes, old_nodes, t):
    ...
    if num_old > 0:
        ...
        # -------------------------------------------------------*
        new2_old1 = (
            (u, v, w) for ((u, v), w) in product(combinations(new_nodes, 2), old_nodes) if w not in [u, v]
        )
        ...
        # -------------------------------------------------------*
        new1_old2 = (
            (u, v, w) for ((u, v), w) in product(combinations(old_nodes, 2), new_nodes) if w not in [u, v]
        )
...
def estimate_motif_type_probs(triplets, role_distr, t):
    ...
    motif_type_probs = (
        helper_estimate_motif_type_probs(role_distr, motif, i) for i in [3, 2, 1] for motif in triplets
    )
...
def helper_estimate_motif_type_probs(role_distr, motif, motif_type):
    ...
    if motif_type == 3:
        prob_triangle = role_distr[u]["equal3"] * role_distr[v]["equal3"] * role_distr[w]["equal3"]
        return prob_triangle
    elif motif_type == 2:
        prob_wedge = (
            (role_distr[u]["hub"] * role_distr[v]["spoke"] * role_distr[w]["spoke"])
        ...
        prob_edge = (
            (role_distr[u]["outlier"] * role_distr[v]["equal2"] * role_distr[w]["equal2"])
...
def sample_motifs(
    ...
    if not file_exists(motifs_t_file):
        ...
        exp_counts = {i: int(np.round(motif_props[i] * num_triplets)) for i in m_types}

        for i in m_types:
            if exp_counts[i] > 0:
                probabilities = [
                    helper_estimate_motif_type_probs(role_distr, motif, motif_type=i)
                    * np.mean([type_counts[node][i] for node in motif])
                ...
                if total > 0.0:
                    ...
                    remaining, num_remaining = helper_get_active_triplets(
                        new_nodes, old_nodes, t, gen_dir, motifs_t
                    )

                    if sample_size > num_nonzero:
                        logging.warning(
                            f"Could not sample expected counts for motif type i={i} and t={t + 1}"
                        )
                        motifs_i = set(
                            [frozenset(m) for idx, m in enumerate(remaining) if probabilities[idx] > 0]
                        )
                        del probabilities
                        gc.collect()
                    else:
                        ...
                        motifs_i_idx = list(
                            np.random.choice(
                                range(num_remaining), sample_size, p=probabilities, replace=False
                            )
                        )
                        del probabilities
                        gc.collect()
                        ...
                        motifs_i = set(
                            [frozenset(m) for idx_m, m in enumerate(remaining) if idx_m in motifs_i_idx]
                        )
                    ...
                    motif_timesteps_i, motif_rates_i = sample_motif_timesteps(
                        num_timesteps, motifs_i, i, motif_type_rates, t, gen_dir
                    )
                    ...
                    role_distr, role_counts, roles_motifs_i = sample_node_roles(
                        motifs_i_show, i, motif_timesteps_i, role_distr, role_counts, gen_dir, gen_dir_t, t
                    )
...
def sample_node_roles(motifs, motif_type, motif_timesteps, role_distr, role_counts, gen_dir, gen_dir_t, t):
    """
    ...
    """
    roles_motifs = {motif: {node: None for node in motif} for motif in motifs}
    nodes_to_update = set()
...
def update_role_distr(nodes, role_distr, role_counts, gen_dir, t, gen_dir_t=None):
    ...
    for node in nodes:
        ...
        role_distr[node] = {r: counts[r] / total if total > 0 and counts[r] > 0 else 0 for r in roles}

    role_distr_file = os.path.join(gen_dir, f"role_distr.msg")
    msgpack_save(role_distr_file, {"role_distr": role_distr, "role_counts": role_counts})
    if gen_dir_t:
...
def get_role_distr(gen_dir, model_params):
    ...
    if file_exists(role_distr_file):  # load updated distribution and counts
        ...
        role_distr = update_role_distr(list(role_counts.keys()), role_distr, role_counts, gen_dir, -2)
...
def sample_motif_timesteps(num_timesteps, motifs, motif_type, motif_type_rates, t, gen_dir):
    ...
    if motifs_to_do:
        ...
        for motif in motifs_to_do:
            ...
            motif_rates[motif] = np.random.exponential(
                motif_type_rates[i]["scale"] + motif_type_rates[i]["loc"]
            )
        ...
        msgpack_save(motif_timesteps_file, {"motif_timesteps": motif_timesteps, "motif_rates": motif_rates})
...
def generate_dynamic_graph(
    ...
    roles = ["equal3", "hub", "spoke", "equal2", "outlier"]
    motif_roles_assigned = {v: {r: 0 for r in roles} for v in range(num_nodes)}
    node_roles_assigned = {v: {t: set([]) for t in range(num_timesteps)} for v in range(num_nodes)}
    ...
    for t in range(num_timesteps):
        ...
        if len(new_nodes) > 0:
            ...
            # Get motif edges
            motif_edges_t, motif_roles_assigned = get_motif_edges(
                motifs_t_show, motif_types_t, roles_t, motif_roles_assigned, t, gen_data_dir
            )
...
def get_generated_graph_data(gen_data_dir, num_timesteps, model_params):
    ...
    if not file_exists(gen_data_file):
        ...
        # Generate dynamic graph
        gen_data = generate_dynamic_graph(
            num_timesteps,
            length_timestep,
            num_nodes,
            node_rate,
    ...
    if "graph edges" not in gen_data.keys():
        ...
        # Get graph edges
        graph_edges = {t: set([]) for t in range(num_timesteps)}
        counts_per_timestep = {t: {i: 0 for i in range(1, 4)} for t in range(num_timesteps)}
...
def create_igraph(gen_data, gen_data_dir):
    ...
    g = igraph.Graph(
        n=gen_data["num nodes"], directed=False, edges=edges, edge_attrs={"timestep": edge_timesteps}
    )
