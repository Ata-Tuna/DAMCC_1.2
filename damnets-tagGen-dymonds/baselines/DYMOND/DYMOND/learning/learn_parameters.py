...
def learn_node_arrival_rates(g, timesteps, tmp_files_dir):
    ...
    if not file_exists(node_arrival_rates_file):
        node_arrivals = {v["nid"]: timesteps.index(v["active"]) for v in g.vs if v["active"]}
            ...
            for edge in g_t.es:
                ...
                for w in nodes[t]:
                    if w != u and w != v:
                        m = frozenset((u, v, w))
                        edges_used = set(
                            [frozenset((e.source, e.target)) for e in g_t.es.select(_within=[u, v, w])]
                        )
                        i = len(edges_used)
...
def calc_motifs_timesteps(motif_types, g, timesteps, tmp_files_dir):
    ...
    if not file_exists(motifs_timesteps_file):
        time_code = log_time()
        motifs_timesteps = {"timesteps": timesteps, "files": {t: None for t in timesteps}}
        ...
        for idx_t, t in enumerate(timesteps):
            ...
            if not file_exists(motifs_t_file):
                ...
                motifs_t = {
                    "counts": {
                        0: int(comb(len(V[t]), 3, repetition=False)) - cnt_1 - cnt_2 - cnt_3,
                        1: cnt_1,
                        2: cnt_2,
                        3: cnt_3,
                    },
...
def get_motif_counts(motifs, motif_types, g, timesteps, tmp_files_dir):
    ...
    if not file_exists(motif_counts_file):
        time_code = log_time()
        motif_counts = {m: {"counts": 0, "timesteps": 0} for m in motifs}
            ...
            if not file_exists(motif_counts_t_file):
                ...
                for i in [3, 2, 1]:
                    for m in motifs_t["motifs"][i]:
                        ...
                        weight = 0
                        edges_motif = [
                            (e.source, e.target, e["weight"]) for e in g_t.es.select(_within=[u, v, w])
                        ]
                        n = len(edges_motif)
...
def learn_motif_interarrival_rates(motifs, motif_types, g, timesteps, tmp_files_dir):
    ...
    if not file_exists(motif_interarrivals_file):
        ...
        m_types = [3, 2, 1]
        rates_distribution = {
            i: [motif_interarrival_rates[m] for m in motifs if motif_types[m] == i] for i in m_types
        }
...
def get_node_role_counts(g, timesteps, tmp_files_dir):
    ...
    if not file_exists(role_counts_file) and not file_exists(type_counts_file):
        ...
        role_counts = {v.index: {r: 0 for r in roles} for v in g.vs}
        type_counts = {v.index: {m_type: 0 for m_type in [3, 2, 1]} for v in g.vs}
...
def learn_parameters(dataset_dir, dataset_info, g):
    ...
    model_params["L"] = dataset_info["L"]

    # Learn node arrival rates
    model_params["node arrivals"] = learn_node_arrival_rates(g, dataset_info["timesteps"], tmp_files_dir)

    # Get motifs in graph
    motifs_graph = get_motifs_graph(g, dataset_info["timesteps"], tmp_files_dir)

    # Learn motif type inter-arrival rates
    model_params["motif interarrivals"] = learn_motif_interarrival_rates(
        motifs_graph["motifs"], motifs_graph["types"], g, dataset_info["timesteps"], tmp_files_dir
    )

    # Learn motif proportions
    model_params["motif proportions"] = learn_motif_proportions(
        motifs_graph["motifs"], motifs_graph["types"], dataset_info["N"], tmp_files_dir
    )

    del motifs_graph  # free up memory

    # Learn node roles distribution
    roles_distr, roles_counts, type_counts = learn_node_roles_distribution(
        g, dataset_info["timesteps"], tmp_files_dir
    )
