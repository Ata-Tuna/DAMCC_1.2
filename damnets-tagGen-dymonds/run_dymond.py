...
def create_dymond_datasets(fstr):
    for entry in tqdm(list(os.scandir(fstr))):
        ...
        g = igraph.Graph(
            n=G.number_of_nodes(), directed=False, edges=edgelist, edge_attrs={"timestep": edge_timesteps}
        )
        for v in g.vs:
...
def parse_arguments():
    ...
    parser.add_argument(
        "-p", "--graphs_path", type=str, help="The path to the graphs on which to train DYMOND."
    )
    parser.add_argument(
