...
class GNNTree(torch.utils.data.Dataset):
    ...
    def process_pair(self, A_1, A_2):
        ...
        for i in range(1, n):
            ...
            idx_row_gnn, idx_col_gnn = np.meshgrid((np.ones(1) * i).astype(np.int8), np.arange(i))
            idx_row_gnn = idx_row_gnn.reshape(-1, 1)
            idx_col_gnn = idx_col_gnn.reshape(-1, 1)
            diffs_idx += [np.concatenate([idx_row_gnn, idx_col_gnn], axis=1)]
            labels += [A_2[idx_row_gnn, idx_col_gnn].flatten().astype(np.uint8)]
            # TODO: make this sparse matrix?? Will probably be faster in most cases and save lot of VRAM
            prev_edges += [A_1[idx_row_gnn, idx_col_gnn].flatten().astype(np.uint8)]
    ...
    def collate_fn(self, batch):
        ...
        data = {}
        data["edges_x"] = torch.cat([bb["edges_x"] + b * n for b, bb in enumerate(batch)], dim=1).long()
        data["edges_y"] = torch.cat([bb["edges_y"] + idx_base[b] for b, bb in enumerate(batch)], dim=1).long()
        ...
        data["labels"] = torch.from_numpy(np.concatenate([bb["labels"] for bb in batch])).float()
        data["prev_edges"] = torch.from_numpy(np.concatenate([bb["prev_edges"] for bb in batch])).float()
