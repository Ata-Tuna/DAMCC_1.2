...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str, 
        default="", 
        help="The data saving path.")
    parser.add_argument(
        "-d",
        "--dataset_path",
        type=str,
        default="",
        help="path to the dataset, stored as lists of networkx graphs. ",
    )
    parser.add_argument(
        "-t",
        "--train_p",
...
def main():
    ...
    save_dir = c_args.save_path
    mkdir(save_dir)

    # graphs_path = os.path.join(save_dir, f"{c_args.dataset_path}")
    graphs = load_graph_ts(c_args.dataset_path)
    dataset_name = c_args.dataset_path.split("/")[-1].split(".")[0]
    if c_args.randomize:
        print("Randomizing")
    ...
    save_graph_list(train_graphs, os.path.join(save_dir, f"{dataset_name}_train_graphs_raw.pkl"))
    save_graph_list(val_graphs, os.path.join(save_dir, f"{dataset_name}_val_graphs_raw.pkl"))
    save_graph_list(test_graphs, os.path.join(save_dir, f"{dataset_name}_test_graphs_raw.pkl"))
    ...
    save_graph_list(train_processed, os.path.join(save_dir, f"{dataset_name}_train_graphs.pkl"))
    del train_processed
    del train_graphs
    gc.collect()

    val_processed = format_data(val_graphs, c_args.num_workers)
    save_graph_list(val_processed, os.path.join(save_dir, f"{dataset_name}_val_graphs.pkl"))
