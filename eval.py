...
def make_ba_plots(sampled_ts, test_ts, model_name, save_dir):
    ...
    df = pd.concat(
        [
            get_final_degrees_df(sampled_ts, model_name, incr=-1),
            get_final_degrees_df(test_ts, "Test", incr=-1),
        ],
        axis=1,
    )
    ax = df.plot(kind="density", xlim=(0, 25), title="Degree Distribution of $G_T$")
    ax.set_xlabel("Degree")
    save_path = os.path.join(save_dir, f"{model_name}_ba_degree.jpg")
...
def make_three_comm_plots(sampled_ts, test_ts, model_name, save_dir):
    ...
    plt.tight_layout()
    plot_community_eval(
        sampled_ts, test_ts, range(comms[1], comms[2]), axs[0], "Third (Decaying)", model_name
    )
    ...
    save_path = os.path.join(save_dir, f"{model_name}_3_comm_densities.jpg")
    plt.savefig(save_path, format="jpg", dpi=1200, bbox_inches="tight")
...
def plot_density(ts, subgraph_nodes, ax, label, col=None):
    ...
    if col is None:
        ...
        ax.fill_between(
            [t for t in range(T)], sample_mean - sample_std, sample_mean + sample_std, color=col, alpha=0.1
        )
...
def plot_community_eval(sampled_ts, true_ts, subgraph_nodes, ax, label, model_name, ylim=None):
    ...
    for t in range(T):
        density_true[t] = np.mean(
            [nx.density(nx.subgraph(true_ts[b][t], subgraph_nodes)) for b in range(len(true_ts))]
        )
...
def plot_top_N(ts, args, tag, N=5):
    ...
    save_path = os.path.join(args.save_dir, f"{tag}_network_vis.jpg")
    plt.savefig(save_path, format="jpg", dpi=1200, bbox_inches="tight")
...
def plot_network_statistics(stats, save_dir="", tag=""):
    ...
    for idx, statistic in enumerate(stat_names):
        ...
        for model in model_names:
            ...
            plot_statistic_density(
                pd.to_numeric(model_data["mean"]), pd.to_numeric(model_data["std"]), axs[idx], model
            )
...
def compute_network_statistics(ts_list, model_name):
    ...
    for stat_name, stat in s_funs.items():
        ...
        index = pd.MultiIndex.from_tuples(
            [(model_name, stat_name, "mean"), (model_name, stat_name, "std"), (model_name, stat_name, "raw")]
        )
        dfs.append(pd.DataFrame([mean, std, raw], index=index, dtype=np.float64).T)
...
#             [(model_name, stat_name, "mean"), (model_name, stat_name, "std"), (model_name, stat_name, "raw")]
#         )
#         dfs.append(pd.DataFrame([mean, std, raw_flat], index=index, dtype=np.float64).T)
    # return pd.concat(dfs, axis=1)
...
def compute_spectral_bipartivity(ts_list):
    ...
    return pd.DataFrame([mean, std, raw], index=index, dtype=np.float64).T
...
def parse_arguments():
    ...
    parser.add_argument("-o", "--output_dir", type=str, help="The directory to save outputs to.")
...
if __name__ == "__main__":
    ...
    if (
        dataset_name[:12] == "3_comm_decay"
        or dataset_name == "3_comm_total_decay"
        or dataset_name == "3_comm_total_decay_small"
    ):
